...
int main(void) {
  ...
  const string sTestingImgRoot = "D:/Database/Sequence/02/";
  // model's name
  ...
  for (const auto& feature : viTwoLayerFeature) {
  ...
  for (const auto& nn : vnnPairs) {
    ...
    // vector of dumper for saving diffirent threshold result
    vector<Plugin::myBBDumper> voDumper(vfThreshold.size());
    // loaded image
    cv::Mat mImg;
    while (oReader >> mImg) {
        ...
        for (auto r : vScanningWindow) {
          ...
          for (size_t i = 0; i < vfThreshold.size(); ++i) {
            if (fMineResult >= vfThreshold.at(i)) {
              voDumper.at(i).AddRectangle(OriginalRect);
            } // if fMineResult
          } // for vfThreshold
        } // for scanning window
      } // for fRatio
